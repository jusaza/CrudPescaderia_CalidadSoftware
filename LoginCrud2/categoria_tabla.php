...
<!DOCTYPE html>
<html lang="es">
<title>Categoria - Pescaderia</title>
<body>
...
if (isset($_POST['btnbuscar'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM categoria_productos WHERE nombre = ?");
    $stmt->bind_param("s", $buscar);
    $stmt->execute();
    $sqlcat = $stmt->get_result();

} else {
    $sqlcat = mysqli_query(
        $conn,
        "SELECT * FROM categoria_productos ORDER BY id ASC LIMIT " . 
...
if (isset($_GET['pag'])) {
    if ($_GET['pag'] > 1) {
        echo '<a class="BotonesTeam4" href="categoria_tabla.php?pag=' . ($_GET['pag'] - 1) . '">Anterior</a>';
    } else {
        echo '<a class="BotonesTeam4" href="#" style="pointer-events: none">Anterior</a>';
    }
} else {
    echo '<a class="BotonesTeam4" href="#" style="pointer-events: none">Anterior</a>';
}
...
if (isset($_GET['pag'])) {
    if ((($pagina) * $filasmax) < $maxusutabla) {
        echo '<a class="BotonesTeam4" href="categoria_tabla.php?pag=' . ($_GET['pag'] + 1) . '">Siguiente</a>';
    } else {
        echo '<a class="BotonesTeam4" href="#" style="pointer-events: none">Siguiente</a>';
    }
} else {
    echo '<a class="BotonesTeam4" href="categoria_tabla.php?pag=2">Siguiente</a>';
}
